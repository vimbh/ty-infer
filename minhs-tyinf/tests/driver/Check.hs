...
runTests exe testdir = do
  ...
  where
    ...
    doCheck ext name test out = do
      ...
      if v
        then do
          diff <- getDiff (filter (not . all isSpace) $ (map (dropWhile isSpace) . lines) out) <$> filter (not . all isSpace) . (map (dropWhile isSpace) . lines) <$> readFile (test `replaceExtension` ".out")
